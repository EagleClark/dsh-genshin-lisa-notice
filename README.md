...
- **飞书 Webhook 地址**：填入飞书群自定义机器人的 webhook 并开启「飞书通知」，即可推送到群
- 点 **确认** 保存并立即生效；**取消** 丢弃未确认的选择；**恢复默认** 一键回到内置默认
- 配置即时生效，无需重启

**3. 听不到声音？** 浏览器自动播放策略要求页面有过一次用户交互（点击/按键）。重启后第一次提醒若被拦截，点一下页面任意处后下次提醒即可正常出声。系统通知需在首次交互时**允许**浏览器通知权限。
