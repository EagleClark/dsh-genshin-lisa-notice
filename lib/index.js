...
export function apply(ctx) {
  ...
  const summarizeEvents = (events) => {
    ...
    for (let i = events.length - 1; i >= 0; i--) {
      const ev = events[i]
      if (!ev) continue
      if (ev.type === 'assistant/message' && result === '') {
        const d = ev.data
        result = textOf(d && d.message && d.message.content)
      }
      if (ev.type === 'user/message' && task === '') {
        const d = ev.data
        task = textOf(d && d.content)
      }
      if (result !== '' && task !== '') break
    }
    const lines = []
    if (task !== '') lines.push('任务：' + clip(task, 80))
    if (result !== '') lines.push('结果：' + clip(result, 200))
    return lines.join('\n')
  }
  ctx.on('agent/status', async (payload) => {
    ...
    const debugPath = join(dshHome, 'data', 'dsh-genshin-lisa-notice', 'debug.log')
    const dbg = (record) => {
      try {
        mkdirSync(join(dshHome, 'data', 'dsh-genshin-lisa-notice'), { recursive: true })
        appendFileSync(debugPath, JSON.stringify(record) + '\n')
      } catch (error) { /* ignore */ }
    }
    ...
    if (typeof sessionId === 'string' && query && typeof query.readSurface === 'function') {
      try {
        ...
        const events = surface && surface.events ? surface.events : undefined
        summary = summarizeEvents(events)
        dbg({ ok: true, sessionId, eventCount: Array.isArray(events) ? events.length : -1, eventsType: Array.isArray(events) && events[0] ? events[0].type : null, summary })
        if (summary) pendingSummary = summary
      } catch (error) {
        // Summary is best-effort; the webhook still fires below.
        dbg({ ok: false, sessionId, error: String(error && error.message ? error.message : error) })
      }
    } else {
      dbg({ ok: false, sessionId, note: 'no sessionQuery or readSurface' })
    }
    // Fire the completion webhook regardless of summary-read success.
    if (completionArmed) {
      completionArmed = false
      const body = (summary || pendingSummary || '').trim()
      sendFeishu(body ? `✅ 任务完成\n${body}` : '✅ 任务完成')
    }
  })
