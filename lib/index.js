...
export function apply(ctx) {
  ...
  ctx.on('agent/status', async (payload) => {
    ...
    if (!query || typeof query.readSurface !== 'function') return
    try {
      const surface = await query.readSurface(sessionId)
      const summary = summarizeEvents(surface && surface.events)
      if (summary) pendingSummary = summary
    } catch (error) {
      // Leave the previous summary; notification is best-effort.
    }
  })
    ...
    host.effect(() => {
      const disposers = [
        ...
        host.webServer.register({
          ...
          handler: (req, res) => serveAudio(res, resolveAudio(audioConfig.completionAudio, 'completion')),
        }),
        host.webServer.register({
          kind: 'exact',
          path: '/dsh-genshin-lisa-notice/interaction.mp3',
          handler: (req, res) => serveAudio(res, resolveAudio(audioConfig.interactionAudio, 'interaction')),
        }),
        host.webServer.register({
          kind: 'exact',
