...
export function apply(ctx) {
  ...
  ctx.on('tools/pre-execute', (exec, next) => {
    if (exec && exec.name === 'ask_user_question') {
      ...
      const s = formatInteraction(question)
  ...
  // Interaction summary: the ongoing task + the question being asked.
  const formatInteraction = (question) => {
    const lines = []
      ...
      const disposers = [
        ...
        host.webServer.register({
          ...
          handler: async (req, res) => {
            ...
            res.end(JSON.stringify({ completion, interaction, summary, interactionSummary, interactionAudio: true }))
          },
        }),
      ]
      return () => {
